<%@ Page Language="C#" AutoEventWireup="true" MasterPageFile="~/Site.Master" CodeBehind="FrmQuotationMaintenance.aspx.cs" Inherits="CUBIC_CIBT_Project.FrmQuotationMaintenance" %>
...
<asp:Content ID="BodyContent" ContentPlaceHolderID="MainContent" runat="server">
	...
	<script>
		...
		function validation() {
			// Drop List Field
			var ddlQOMode = $('#<%= ddlQuotationMode.ClientID %>').val();
			...
			var QuotationDate = $('#<%= txtQuotationDate.ClientID %>').val();


			// Validate Quotation Mode
			if (!ddlQOMode || ddlQOMode.trim() === "") {
				...
				if (ddlQOMode === "U" && (!DrpListRevisionNo || DrpListRevisionNo.trim() === "")) {
					showErrorModal('Failed', 'Kindly select the Revision No.');
					return;
				}

				//Validate Revision No[Text] (only in Create Mode)
				if (ddlQOMode === "C" && (!RevisionNo || RevisionNo.trim() === "")) {
				...
				// Parse and Validate the logical of the Quotation Date 
				const inputQuotationDate = new Date(QuotationDate);
				if (isNaN(inputQuotationDate.getTime())) {
					showErrorModal('Failed', 'Please enter a valid Quotation Date.');
					return;
				}
				...
				// Validate the range (100 years ago < Quotation Date  <= Current Date)
				if (inputQuotationDate > currDate || inputQuotationDate < minDate) {
					showErrorModal('Failed', 'The Quotation Date must be between today and 100 years ago.');
					return;
				}
			}
			return;
		...
		document.title = "Quotation Maintenance";

		//Data Table Filter
		$(document).ready(function () {
			var table = $('#QOMTable').DataTable({
				"paging": true,
				"searching": true,
				"info": true,
				"destroy": true
	...
	<asp:UpdatePanel ID="UpdatePanelLotInfor" runat="server">
		<ContentTemplate>
			...
			<div class="container-fluid">
				...
				<p class="mb-4"></p>
				<!-- Document -->
				<div class="card shadow mb-4" runat="server" ID="E_QoM" Visible="false">
					<div class="card-header py-3">
						<h6 class="m-0 page-heading-cubic"><i class="fa fa-file"></i>&nbsp;&nbsp;Quotation Maintenance</h6>
					</div>
					...
					<div class="card-body">
						...
						<div class="row row-margin-btm-cubic">
							<div class="form-group col-md-4" style="border: 0px solid">
								<asp:Label ID="lblQuotationddl" runat="server" Text="Select Mode" class="input-label-cubic"></asp:Label>
								<asp:DropDownList ID="ddlQuotationMode" runat="server" AutoPostBack="true" class="form-control" ValidateRequestMode="Enabled" BackColor="white" Style="border: 1px solid; font-size: 16px; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; min-width: 140px;" OnSelectedIndexChanged="ddlQuotationMode_SelectedIndexChanged">
						...
						<br />
						<%-- row Quotation Date/Status--%>
						<div class="row row-margin-btm-cubic">

							<div class="form-group col-md-4">
								<asp:Label ID="lblQuotationDate" runat="server" Text="Quotation Date" class="input-label-cubic"></asp:Label>
								<asp:TextBox ID="txtQuotationDate" runat="server" TextMode="Date" class="form-control"></asp:TextBox>
							</div>
							<div class="form-group col-md-2">
								<asp:RequiredFieldValidator ID="RequiredFieldValidator2" runat="server" ControlToValidate="txtQuotationDate" ErrorMessage="Please enter Quotation Date" ForeColor="Red" BorderStyle="None">*</asp:RequiredFieldValidator>
							</div>
							<div class="form-group col-md-4">
				...
				<!-- DataTales -->
				<div class="card shadow mb-4" runat="server" ID="V_QoM" Visible="false">
					<div class="card-header py-3">
						<h6 class="m-0 page-heading-cubic"><i class="fa fa-file"></i>&nbsp;&nbsp;Quotation Table</h6>
					</div>
					<div class="card-body">
						<div class="row row-margin-btm-cubic">
							<div class="col">
												...
												<div style="overflow-x: scroll; height: 100%; min-width: 350px;">
													<div class="datatable">
														<div id="dataTable_wrapper" class="dataTables_wrapper dt-bootstrap4">
															<asp:Repeater ID="QOMRepeater" runat="server">
																<HeaderTemplate>
																	<%-- Filter --%>
																	<div class="row mb-3">
																		<!-- Status Filter -->
																	...
																	<table id="QOMTable" class="table table-bordered table-hover table-striped mydatatable " style="width: 100%">
